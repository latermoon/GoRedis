...
func Open(dbname string, o *Options) (*DB, error) {
	...
	// defer C.rocksdb_free(unsafe.Pointer(ldbname))
...
func (db *DB) GetApproximateSizes(ranges []Range) []uint64 {
	...
	// for i := range ranges {
	// C.rocksdb_free(unsafe.Pointer(starts[i]))
	// C.rocksdb_free(unsafe.Pointer(limits[i]))
	// }
	return sizes
}
...
func (db *DB) PropertyValue(propName string) string {
	...
	// C.rocksdb_free(unsafe.Pointer(cname))
	return value
}
